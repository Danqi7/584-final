...
class STSEval(object):
    def loadFile(self, fpath):
        ...
        for dataset in self.datasets:
            if hasattr(self, 'STS16') == True:
                lines = io.open(fpath + '/STS2016.input.%s.txt' % dataset,
                                encoding='utf8').read().splitlines()
                #print(lines[0])
                #print(lines[0].split("\t"))
                #print(len(lines[0].split("\t")))
                debug_lines = [l.split("\t") for l in
                ...
                raw_scores = np.array([x for x in
                                       io.open(fpath + '/STS2016.gs.%s.txt' % dataset,
                                               encoding='utf8')
                                       .read().splitlines()])
            else:
